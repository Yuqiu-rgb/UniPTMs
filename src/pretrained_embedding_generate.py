...
def get_T5_model():
    # tokenizer = T5Tokenizer.from_pretrained(r'C:\Users\admin\Desktop\prot_t5_xl_half_uniref50-enc')
    # model = T5EncoderModel.from_pretrained(r'C:\Users\admin\Desktop\prot_t5_xl_half_uniref50-enc')
...
def get_embeddings(model, tokenizer, seqs, per_residue,
    ...
    for seq_idx, (pdb_id, seq) in enumerate(seq_dict, 1):
        ...
        if len(batch) >= max_batch or n_res_batch >= max_residues or seq_idx == len(
            ...
            token_encoding = tokenizer.batch_encode_plus(seqs, add_special_tokens=True,
                                                         padding=True)
            ...
            for batch_idx, identifier in enumerate(
                    pdb_ids):  # for each protein in the current mini-batch
...
def embedding_out(train_data, test_data, per_residue, max_residues=400, max_seq_len=100,
                  max_batch=100, path=False):
    ...
    results = get_embeddings(model, tokenizer, train, per_residue, max_residues, max_seq_len,
                             max_batch)
    model.cpu()
    del model
...
if __name__ == "__main__":
    ...
    # ----------------- For PH dataset ---------------------
    # np.save('../embedding/PH_train_embedding.npy', x_train_encoding)
    # np.save('../embedding/PH_test_embedding.npy', x_test_encoding)
    # # ------------------------------------------------------
